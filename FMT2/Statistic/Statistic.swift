...
class Statistic {
    ...
    var introductionMistakeCount: Int {
        ...
        return introductionSimpleStagesPassing.flatMap{$0.exercises}.reduce(0){$0 + $1.errors.count}
    }
    ...
    var exerciseProgress: (Int, Int) {
        ...
        let passedCount = currentExercisesSimpleStagesPassing.reduce(0){$0 + $1.index}
        let totalCount = currentExercisesSimpleStagesPassing.reduce(0){$0 + $1.exercises.count}
        return (passedCount, totalCount)
    }
    
    var exerciseElapsedTime: TimeInterval {
        updateExercisesSimpleStagesPassingIfNeeded()
        return exercisesSimpleStagesPassing.flatMap{$0.exercises}.flatMap{$0.elapsedTime}.map{$0.seconds}.reduce(0.0){$0 + $1}
    }
    
    var exerciseMistakeCount: Int {
        updateExercisesSimpleStagesPassingIfNeeded()
        return exercisesSimpleStagesPassing.flatMap{$0.exercises}.flatMap{$0.errors.count}.reduce(0){$0 + $1}
    }
    
    var examElapsedTime: TimeInterval {
        updateExamStagesPassingIfNeeded()
        return examStagesPassing.flatMap{$0.exercises}.flatMap{$0.elapsedTime}.map{$0.seconds}.reduce(0.0){$0 + $1}
    }
    
    var examPassedCount: Int {
        updateExamStagesPassingIfNeeded()
        return examStagesPassing.reduce(0){$0 + $1.index}
    }
    
    var examMistakeCount: Int {
        updateExamStagesPassingIfNeeded()
        return examStagesPassing.flatMap{$0.exercises}.flatMap{$0.errors.count}.reduce(0){$0 + $1}
    }
    ...
    static var byDays: [GraphStatistic] {
        ...
        let allRightAnswers = allExamExercises.filter{$0.isPassed && !$0.passedAt.afterDay(date: curDate)}.map{$0.passedAt}.sorted(by: {$0 > $1})
        var indRA = 0
        let allMisatakes = allExamExercises.flatMap{$0.errors}.filter{!$0.date.afterDay(date: curDate)}.sorted(by: {$0.date > $1.date})
        var indM = 0
        let allElapsedTime = allExamExercises.flatMap{$0.elapsedTime}.filter{!$0.createdAt.afterDay(date: curDate)}.sorted(by: {$0.createdAt > $1.createdAt})
        var indET = 0
